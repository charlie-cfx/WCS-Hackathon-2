...
CREATE TABLE IF NOT EXISTS `emmaus_phone`.`OS` (
  ...
  CONSTRAINT `fk_OS_OS_version1`
    FOREIGN KEY (`OS_version_id`)
    REFERENCES `emmaus_phone`.`OS_version` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `emmaus_phone`.`phone` (
  ...
  PRIMARY KEY (`id`, `brand_id`, `model_id`, `OS_id`, `accessory_id`, `state_id`),
  ...
  CONSTRAINT `fk_phone_brand`
    ...
    REFERENCES `emmaus_phone`.`brand` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_phone_model1`
    FOREIGN KEY (`model_id`)
    REFERENCES `emmaus_phone`.`model` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_phone_OS1`
    FOREIGN KEY (`OS_id`)
    REFERENCES `emmaus_phone`.`OS` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_phone_accessory1`
    FOREIGN KEY (`accessory_id`)
    REFERENCES `emmaus_phone`.`accessory` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_phone_state1`
    FOREIGN KEY (`state_id`)
    REFERENCES `emmaus_phone`.`state` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
